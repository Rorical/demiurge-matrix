...
import SorterWasmNoSIMDUrl from './sorter_no_simd.wasm?url';
import SorterWasmNonSharedUrl from './sorter_non_shared.wasm?url';
import SorterWasmNoSIMDNonSharedUrl from './sorter_no_simd_non_shared.wasm?url';
...
async function ensureWasmLoaded(url, cache) {
    if (!cache.value) {
        cache.value = await loadWasmAsBase64(url);
    }
    return cache.value;
}
...
const wasmCache = {
    sorter: { value: null },
    sorterNoSIMD: { value: null },
    sorterNonShared: { value: null },
    sorterNoSIMDNonShared: { value: null }
};
...
export async function createSortWorker(splatCount, useSharedMemory, enableSIMDInSort, integerBasedSort, dynamicMode,
    ...
    if (!enableSIMDInSort && !useSharedMemory) {
        sourceWasm = SorterWasmNoSIMDUrl;
        cacheKey = wasmCache.sorterNoSIMD;
        ...
        if (iOSSemVer && iOSSemVer.major <= 16 && iOSSemVer.minor < 4) {
            sourceWasm = SorterWasmNoSIMDNonSharedUrl;
            cacheKey = wasmCache.sorterNoSIMDNonShared;
        }
        ...
        if (iOSSemVer && iOSSemVer.major <= 16 && iOSSemVer.minor < 4) {
            sourceWasm = SorterWasmNonSharedUrl;
            cacheKey = wasmCache.sorterNonShared;
        } else {
            sourceWasm = SorterWasmUrl;
            cacheKey = wasmCache.sorter;
        }
    } else {
        sourceWasm = SorterWasmUrl;
        cacheKey = wasmCache.sorter;
    ...
    // 加载 WASM 文件为 base64
    const sourceWasmBase64 = await ensureWasmLoaded(sourceWasm, cacheKey);

    const sorterWasmBinaryString = atob(sourceWasmBase64);
