...
export default class Lipsync extends EventTarget {
	...
	static SENSITIVITY_THRESHOLD: number = .43 // range [-.5, .5]
	...
	private _blendShapes: { blendShapeKiss: number, blendShapeLips: number, blendShapeMouth: number }
	...
	play() {
		...
		for (let m = 0; m < Lipsync.VOICE_BOUNDING_FREQUENCIES.length; m++) {
			this._indicesFrequency[m] = Math.round(((2 * Lipsync.SAMPLE_COUNT) / Lipsync.SAMPLING_FREQUENCY) * Lipsync.VOICE_BOUNDING_FREQUENCIES[m])
		}
		...
		if (!this._useAudioWorklet) {
			this._node = this.AUDIO_CONTEXT.createScriptProcessor(Lipsync.SAMPLE_COUNT * 2, 1, 1) as ScriptProcessorNode
	...
	private initProcessor() {
		...
		return this.AUDIO_CONTEXT.audioWorklet.addModule(new URL('@/libs/lipsync/processor.js', import.meta.url)).then(() => {
	...
	private _getData(url: string): Promise<AudioBuffer> {
		...
		return new Promise((resolve, reject) => {
			request.onload = () => {
				...
				this.AUDIO_CONTEXT.decodeAudioData(audioData, (_buffer: AudioBuffer) => {
