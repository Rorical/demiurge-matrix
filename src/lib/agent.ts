...
export class Agent {
  ...
  async run(userInput: string, options: AgentRunOptions = {}): Promise<AgentRunResult> {
    ...
    while (iterations < maxRecursions) {
      ...
      const response: any = await this.client.sendChat(this.history, { model: this.model, stream: options.stream });
      lastRaw = response;
  ...
  private normalizeAssistantMessage(message: any): { content: string; toolCalls: ToolCallRequest[] } {
    ...
    const toolCalls: ToolCallRequest[] =
      (message.tool_calls as OpenRouterToolCall[] | undefined)?.map((toolCall) => {
        ...
        try {
          ...
          parsedArgs = { error: 'Failed to parse tool arguments', raw: toolCall.function.arguments };
        }
        return {
          id: toolCall.id,
