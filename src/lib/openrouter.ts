...
export class OpenRouterClient {
  ...
  constructor(options: OpenRouterClientOptions = {}) {
    ...
    if (!apiKey) {
      throw new Error('OpenRouter API key is missing. Save it in settings or pass apiKey to OpenRouterClient.');
    }
      ...
      switch (message.role) {
        ...
        case 'tool': {
          if (!message.toolCallId) {
            throw new Error('Tool messages require a toolCallId for OpenRouter compatibility.');
          }
